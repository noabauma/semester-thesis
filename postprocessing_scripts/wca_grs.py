...
def load_particles(debug):
    if debug:
        ...
        cut_per = 0.95   #how much to be cut [0.0 = all gone, 1.0 = all stay]
        cut = h*cut_per
        deleteable = []
        for i in range(n):
...
if __name__ == "__main__":
    ...
    print("bins radius = ", r_bins)
    print("bins xy-location spaces = ", bins_loc[1] - bins_loc[0])
    ...
    for i in range(N_bins):
        densities_ = density_profile[i,z_0_loc:]
        bins_ = bins_step[z_0_loc:]    
        
        keep = (0.0167 <= densities_) & (densities_ <= 0.0368)

        densities = densities_[keep]
        bins      = bins_[keep]
        ...
        if densities.shape[0] > 0:
            ...
            plt.plot(bins_step, converter*density_profile[i,:], color="#aaaaaa", marker="x")
            plt.plot(bins, converter*densities, "mx")
            plt.plot(bins_step, converter*sigmoidal(bins_step, popt[0], popt[1], popt[2]), "g-")
            plt.vlines(z_0, 0.0, converter*popt[0], color="#000000", linestyles="--")
            plt.xlabel(r'$z \: [\AA]$')
    ...
    #plt.plot(sigmoidal_fit[:,0], sigmoidal_fit[:,1])
    #plt.show()
