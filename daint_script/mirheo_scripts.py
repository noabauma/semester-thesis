...
# WCA on GRS
########################
def droplet_grs(A_eps, B, sigma, rc_cw, data_job_task_id):
    ##############################
    ...
    if debug:
        dump_every = 100
        u.registerPlugins(mir.Plugins.createDumpParticles('part_dump_w', pv_w, dump_every, [], data_job_task_id + '/h5_droplet_grs/pv_w-'))
    ...
    if u.isComputeTask():
        ...
        all_files = glob.glob(data_job_task_id + "/h5_droplet_grs/pv_w-*.h5")
        ...
        for file_ in range(n_files):
            ...
            while(True):
                ...
                if abs(old_slope - slope) < 1e-5  or z_f < -4000.0:
                    break
...
# WCA in CNT
########################
def droplet_cnt(A_eps, B, sigma, rc_cw, data_job_task_id):
    ##############################
    # Stage 0. Initialize Mirheo
    ##############################
    u = mir.Mirheo(ranks, domain, debug_level=3, log_filename='log', comm_ptr=MPI._addressof(MPI.COMM_WORLD))
    ...
    if debug:
        dump_every = 100
        u.registerPlugins(mir.Plugins.createDumpParticles('part_dump_w', pv_w, dump_every, [], data_job_task_id + '/h5_droplet_cnt/pv_w-'))
    ...
    if u.isComputeTask():
        ...
        ###STEP 4.0: load particles in either debug mode or from h5file###
        all_files = glob.glob(data_job_task_id + "/h5_droplet_cnt/pv_w-*.h5")
        all_files = np.sort(all_files)
        n_files   = len(all_files)
        for file_ in range(n_files):
            particles = h5py.File(all_files[file_], "r")["position"][()]
...
########################
def water_in_cnt(A_eps, B, sigma, rc_cw, data_job_task_id):
    ##############################
    # Stage 0. Initialize Mirheo
    ##############################
    ...
    ##############################
    path_ = data_job_task_id + "/h5_w_in_cnt/"
    dump_every = 1

    ####Viscosity####
    mask = '011101110'   #"Pxx", "Pxy", "Pxz", "Pyx", "Pyy", "Pyz", "Pzx", "Pzy", "Pzz"
    u.registerPlugins(mir.Plugins.createStressTensor('stress_tensor', pv_w, dump_every, mask, path_))

    ####Friction coef.####
    u.registerPlugins(mir.Plugins.createTotalForceSaver('tangential_force', pv_w, dump_every, path_))

    if debug:
        dump_every = 100
        u.registerPlugins(mir.Plugins.createDumpParticles('part_dump_w', pv_w, dump_every, [], path_ + 'pv_w-'))
    ...
    if u.isComputeTask():
        ...
        P = np.loadtxt(data_job_task_id + "/h5_w_in_cnt/stress_tensor.csv", dtype=np.float64, comments="t", delimiter=",") #t, xy, yx, yz, zy, zx, xz
        t = P[:, 0]  * ureg.fs
        ...
        P_cp = np.loadtxt(data_job_task_id + "/h5_w_in_cnt/stress_tensor.csv", dtype=np.float64, comments="t", delimiter=",") #t, xy, yx, yz, zy, zx, xz
        ...
        coord = np.loadtxt(data_job_task_id + "/h5_w_in_cnt/stress_tensor.csv", dtype=str, delimiter=",", max_rows=1)
        coord = coord[1:]

        if debug:
            print(coord)
            print("Loading F", flush=True)
        F = np.loadtxt(data_job_task_id + "/h5_w_in_cnt/tangential_force.csv", dtype=np.float64, comments="t", delimiter=",") #t, x, y, z
        ...
        p = np.polyfit(x.m_as(ureg.s)[1000:], eta.m_as(ureg.mPa*ureg.s**2)[1000:], deg=1)
        ...
        return p[0], lambda_.m_as(ureg.N * ureg.s / (ureg.m**3))
