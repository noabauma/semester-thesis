...
if __name__ == "__main__":
    ...
    water = np.loadtxt("../particle_generators/water_cube_78408.txt")
    ...
    water_n = water.shape[0]
    cnt_n = cnt.shape[0]
    
    shift = np.array([200.0, 200.0, 110.0 - L_cnt*0.5])
    cnt += shift

    shift = np.array([200.0 - np.mean(water[:,0]), 200.0 - np.mean(water[:,1]), 110.0 - np.mean(water[:,2])])
    water += shift

    
    dt = ureg('2 fs')            		   # timestep   (Werder et al. 2003)
    rc_ww = ureg('0.432 nm')               # Molinero et al.
    rc_cw = ureg('10.0 angstrom')          # 10A was normaly used by (Werder et al. 2003)
    ...
    ic = mir.InitialConditions.FromArray( pos=water, vel=np.random.normal(scale=372e-5, size=(water_n, 3)))
    ...
    #debug
    dump_every = 20000
    u.registerPlugins(mir.Plugins.createDumpParticles('part_dump_c', pv_c, 1000*dump_every, [], 'h5_cnt_water2/pv_c-'))
    u.registerPlugins(mir.Plugins.createDumpParticles('part_dump_w', pv_w, dump_every, [], 'h5_cnt_water2/pv_w-'))
